...
{ /* Ходилка по пустому пространству, но с вероятностью нахождения монеток и магазина для покупок.
    ...
    */
    class Program
    {
        ...
        static void Status(int score, int eat, int dress, int weapon, int coinsLeft, int steps)
        {
        ...
        static void Main(string[] args)
        {
            ...
            while ((action = Console.ReadKey(true)).Key != ConsoleKey.Escape || mCount != 0) // игра продолжается в случае, если не нажата клавиша Escape и оставшихся монет не равно нулю
            {
                noway = false;
                switch (action.Key)
                {
                    case ConsoleKey.UpArrow: Message($"Forward {steps}");
                        if (y > 0)
                        {
                            if (map[y - 1, x] != '░') { y = y - 1; forward_step++; steps++; }
                            else noway = true;
                        }
                        else noway = true;
                        break;
                    case ConsoleKey.DownArrow: Message($"Backwards {steps}");
                        if (y + 1 < ymax)
                        {
                            if (map[y + 1, x] != '░') { y = y + 1; backwards_step++; steps++; }
                            else noway = true;
                        }
                        else noway = true;
                        break;
                    case ConsoleKey.LeftArrow: Message($"Left {steps}");
                        if (x > 0)
                        {
                            if (map[y, x - 1] != '░') { x = x - 1; left_step++; steps++; }
                            else noway = true;
                        }
                        else noway = true;
                        break;
                    case ConsoleKey.RightArrow: Message($"Right {steps}");
                        if (x + 1 < xmax)
                        {
                            if (map[y, x + 1] != '░') { x = x + 1; right_step++; steps++; }
                            else noway = true;
                        }
                        else noway = true;
                        break;
                ...
                Status(score, food, dress, weapon, mCount, steps);
